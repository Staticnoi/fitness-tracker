...
type Action =
  ...
  | {
      ...
      payload: { workoutDayId: string; startedAt: number };
    }
  | {
      type: "UPDATE_WORKOUT_DRAFT";
      payload: {
        workoutDayId: string;
        exerciseIndex: number;
        sets: CompletedWorkout["exercises"][number]["sets"];
        notes: string;
        startedAt: number;
      };
    }
...
function checkAndUnlockAchievements(
  ...
  const unlock = (id: string, progress?: number) => {
    ...
    if (
      !a.unlocked &&
      (progress === undefined || progress >= (a.maxProgress ?? 1))
    ) {
      a.unlocked = true;
  ...
  if (newWorkout) {
    ...
    state.completedWorkouts.slice(0, -1).forEach((workout) =>
      workout.exercises.forEach((exercise) => {
        ...
        previousBest.set(
          exercise.name,
          Math.max(previousBest.get(exercise.name) ?? 0, volume),
        );
      }),
    );
  ...
  const plannedDayIndexes =
    state.workoutPlan?.days.map((day) => dayOrder.indexOf(day.dayOfWeek)) ?? [];
  const scheduledThisWeek = state.dailyQuests.filter(
    (quest) =>
      new Date(`${quest.dateKey}T12:00:00`) >= monday &&
      quest.dateKey <= dateKey(),
  );
  const weekScheduleReached =
    plannedDayIndexes.length > 0 &&
    Math.max(...plannedDayIndexes) <= currentDayIndex;
  if (
    weekScheduleReached &&
    scheduledThisWeek.filter((quest) => quest.status === "completed").length >=
      plannedDayIndexes.length
  )
    unlock("no_skip_week");
...
function calculateStreak(
  ...
  if (!state.lastWorkoutDate)
    return { streak: 1, longest: Math.max(1, state.longestStreak) };
  ...
  switch (action.type) {
    ...
    case "COMPLETE_ONBOARDING": {
      ...
      return {
        ...
        bodyWeightHistory: [
          { date: Date.now(), weight: action.payload.weight },
        ],
        progression: buildProgression([], 0),
        dailyQuests: [questForDate(plan)].filter(
          Boolean,
        ) as AppState["dailyQuests"],
        recoveryChain: null,
        systemEvents: [],
        lastQuestSyncDate: dateKey(),
        firedReminderKeys: [],
    ...
    case "ADD_COMPLETED_WORKOUT": {
      const isActiveQuest = state.dailyQuests.some(
        (quest) =>
      ...
      const isActiveRecovery =
        state.recoveryChain?.objectives.some(
          (objective) =>
            objective.workout.id === action.payload.workoutDayId &&
            objective.status === "active",
        ) ?? false;
      const streakResult =
        isActiveQuest || isActiveRecovery
      ...
      const scaledPlan =
        progressed.workoutPlan && progressed.userProfile
          ? scaleWorkoutPlan(
              progressed.workoutPlan,
              progressed.userProfile,
      ...
      const newlyUnlocked = achievements.filter(
        (item) =>
          item.unlocked &&
          !state.achievements.find((previous) => previous.id === item.id)
            ?.unlocked,
      );
      ...
      const {
        [action.payload.workoutDayId]: completedSession,
        ...activeWorkoutSessions
      } = progressed.activeWorkoutSessions;
      return {
        ...progressed,
        workoutPlan: scaledPlan,
        activeWorkoutSessions,
        achievements,
        systemEvents: [...recordEvents, ...progressed.systemEvents].slice(
          0,
          30,
        ),
      };
    ...
    case "ADD_BODY_WEIGHT": {
      ...
      return {
        ...newState,
        achievements,
        systemEvents: [...recordEvents, ...state.systemEvents].slice(0, 30),
      };
    }
    case "UPDATE_PROFILE": {
      ...
      const today = dateKey();
      const retainedQuests = state.dailyQuests.filter(
        (quest) => quest.dateKey !== today || quest.status !== "active",
      );
      const hasClearedToday = retainedQuests.some(
        (quest) => quest.dateKey === today && quest.status === "completed",
      );
      const replacementQuest = hasClearedToday
        ? null
        : questForDate(plan, today);
      return {
        ...state,
        userProfile: updatedProfile,
        workoutPlan: plan,
        nutritionPlan: nutrition,
        dailyQuests: [...retainedQuests, replacementQuest].filter(
          Boolean,
        ) as AppState["dailyQuests"],
        lastQuestSyncDate: dateKey(),
        activeWorkoutSessions: {},
    ...
    case "START_WORKOUT_SESSION":
      if (state.activeWorkoutSessions[action.payload.workoutDayId])
        return state;
      return {
        ...state,
        activeWorkoutSessions: {
    ...
    case "UPDATE_WORKOUT_DRAFT": {
      const session = state.activeWorkoutSessions[
        action.payload.workoutDayId
      ] ?? {
        workoutDayId: action.payload.workoutDayId,
        startedAt: action.payload.startedAt,
    ...
    case "CLEAR_WORKOUT_SESSION": {
      const { [action.payload]: clearedSession, ...activeWorkoutSessions } =
        state.activeWorkoutSessions;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    loadAppState()
      .then((loaded) => {
        if (loaded)
          dispatch({ type: "LOAD_STATE", payload: syncQuests(loaded) });
      })
      .catch((err: unknown) => {
        if (err instanceof StorageError && err.message.includes("corrupted")) {
          console.error("App state is corrupted, starting fresh:", err.cause);
        } else {
  ...
  useEffect(() => {
    if (!isLoading) {
      saveAppState(state).catch((err: unknown) => {
        saveFailureCount.current += 1;
        console.error(
          `Failed to persist app state (attempt ${saveFailureCount.current}):`,
          err instanceof StorageError ? err.cause : err,
        );
      });
    }
  }, [state, isLoading]);
  ...
  const startWorkoutSession = useCallback((workoutDayId: string) => {
    dispatch({
      type: "START_WORKOUT_SESSION",
      payload: { workoutDayId, startedAt: Date.now() },
    });
  }, []);
  ...
  const updateWorkoutDraft = useCallback(
    (
      workoutDayId: string,
      exerciseIndex: number,
      sets: CompletedWorkout["exercises"][number]["sets"],
